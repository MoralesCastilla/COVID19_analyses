...
env<-read.csv('env_means_2020.02.24_to_2020.03.21.csv')
hum<-read.csv('Human_data.csv')
lock<-read.csv('Lockdown_Mar_28.csv')
...
j=2
	...
	covid.tem <- m3[, c("r", "Temperature.C", "Population", "Route.density", "House.density", "Lockdown")]
...
###############################
	...
	rf_hum<-randomForest(r~ Population+
					...
					Specific.humidity+
					 House.density +
					Route.density+
					Lockdown,data=covid.hum, importance=T)
	...
	rf_tem<-randomForest(r~ Population+
					Temperature.C+
					 House.density +
					Route.density+
					Lockdown,data=covid.tem, importance=T)
	rf_tem	
	...
	op <- par(mfrow=c(2, 3))
		...
		partialPlot(rf_hum, covid.hum, main=NULL, impvar_hum[m], ylab='Fitted function',xlab=impvar_hum[m],
		ylim=c(0,1.1))}
	dev.off()
	...
	op <- par(mfrow=c(2, 3))
		...
		partialPlot(rf_tem, covid.tem, main=NULL, impvar_tem[m], ylab='Fitted function',xlab=impvar_tem[m],
		ylim=c(0,1.1))}
	dev.off()
...
for(i in 1:iterations){
  ...
  #NEW METHOD
  covid.hum$autocor.noise <- autocorrelated_variable(covid.coordinates)
  covid.tem$autocor.noise <- autocorrelated_variable(covid.coordinates)
...
boxplot(IncMSE.df.hum, 
        ...
        notch = TRUE,
        col = viridis(ncol(covid.hum), alpha = 0.7)
        )
...
boxplot(IncMSE.df.tem, 
        ...
        xlab = "% Increment in MSE", 
        notch = TRUE,
        col = viridis(ncol(covid.tem), alpha = 0.7)
        )
